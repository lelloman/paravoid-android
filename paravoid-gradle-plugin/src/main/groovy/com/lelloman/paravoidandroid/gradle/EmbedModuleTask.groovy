...
abstract class EmbedModuleTask extends DefaultTask {
    ...
    void embed() {
        if (bundles.files.size() != 1) {
            throw new GradleException('Each shell variant requires exactly one paravoid module bundle.')
        }
        File target = outputDirectory.file('paravoid/module.zip').get().asFile
