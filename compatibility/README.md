...
| Hilt plugin + Java annotation processing | `hilt`, pinned 2.57.2; Hilt Workers with explicit initialization below | Fragments/Views, other versions |
...
| Hilt Worker + custom WorkManager configuration | [Hilt Work probe](hilt-work/README.md): explicit initialization passes cold injected worker and Room across three processes; lazy `Configuration.Provider` passes normal but fails shell | Automatic provider lookup is unsupported; retries/chains, CoroutineWorker, foreground work, other versions |
