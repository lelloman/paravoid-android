...
| Downloaded DEX cold-start experiment | [Cold-DEX probe](provisioning/COLD-DEX.md): 106 stages on API 30 and 36.1; actual separately compiled A/B execution through Paravoid's loader, unchanged APK, no hot swap, offline execution, tampering rejection and failure without automatic rollback | Single-process entry method, not whole-app lifecycle replacement; no downloaded resources/native code, crash/multiprocess protocol, database rollback or production active-state repair |
...
| Production embedded-resource shell | [Production gate](automatic-resources/PRODUCTION.md): signed plugin APK, real resource/Java-resource loading; 25 stages each on API 30/36.1, including named worker, early ServiceLoader, APK A/B/A replacement and cache rejection | Explicit embedded-only APK task, not full VPK/activation; native relocation has its separate gate below; no retention/recovery UI, ARM64 or release-device coverage |
| Plugin-generated resource split | [Generated-pair gate](automatic-resources/README.md): ordinary AGP app/library R/styleables, pinned-only installed table, A/B/A resources/assets, early Application/provider and named-worker access, recreation/night changes; 19 stages each on API 30/36.1 | Fixture-only APK assembly/early loader; fixed code; no production coherent VPK activation, Compose/system-resource cases, ARM64 or release coverage |
...
| Relocated native payload | [Native payload gate](jni/PAYLOAD.md): 104 JNI assertions plus cache rejection/repair on API 30/36.1 x86_64 and API 30 x86-only/32-bit; fixed installed ABI markers, no installed app-library fallback | Embedded-only; ARM/16 KiB execution, NativeActivity/explicit installed paths, full contract/activation/retention remain unproven |
...
Complete packaging integration beyond the embedded-resource APK stage remains a core implementation item. This
