...
class PackagingTest {
    ...
    @Test void packagesDexAsAssetAndReusesTaskOutputs() {
        ...
        assertEquals(TaskOutcome.SUCCESS, first.task(':module:bundleDebugVoidModule').outcome)
        ...
        assertEquals(TaskOutcome.UP_TO_DATE, second.task(':module:bundleDebugVoidModule').outcome)
        assertEquals(TaskOutcome.UP_TO_DATE, second.task(':shell:embedDebugVoidModule').outcome)
    }

    @Test void rejectsMissingEntryPoint() {
        File root = fixture()
        new File(root, 'module/build.gradle') << "\nvoidModule.entryPoint = 'example.Missing'\n"
        assertTrue(runner(root, ':module:bundleDebugVoidModule').buildAndFail().output
    ...
    private File fixture() {
        ...
        if (!sdk) {
            ...
            File localFile = new File(System.getProperty('void.repo'), 'local.properties')
        ...
        write(root, 'module/build.gradle', '''
            ...
            voidModule { entryPoint = 'example.Entry' }
        ''')
        ...
        write(root, 'shell/build.gradle', '''
            ...
            dependencies { voidBundle project(':module') }
        ''')
    ...
    private static GradleRunner runner(File root, String task) {
        GradleRunner.create().withProjectDir(root).withPluginClasspath()
            .withArguments(task, '--stacktrace', '--max-workers=2',
                '--gradle-user-home', System.getProperty('void.gradleUserHome'))
    }
