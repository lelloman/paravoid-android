...
abstract class PackageApplicationTask extends DefaultTask {
    static final String BASE = 'com/lelloman/voidandroid/runtime/VoidAndroidApplication'
    static final String DELEGATE = 'com/lelloman/voidandroid/runtime/PayloadApplication'
    ...
    @TaskAction void pack() {
        ...
        if (app) {
            ...
            if (bytes == null || new ClassReader(bytes).superName != BASE) {
                throw new GradleException('The manifest Application must directly extend VoidAndroidApplication for this experiment.')
            }
        }
        ...
        new ZipOutputStream(new FileOutputStream(shell)).withCloseable { host ->
            new ZipOutputStream(new FileOutputStream(payload)).withCloseable { module ->
                classes.each { name, bytes ->
                    if (name.startsWith('com/lelloman/voidandroid/runtime/') || name.startsWith('com/lelloman/voidandroid/api/')) {
