...
abstract class BundleModuleTask extends DefaultTask {
    ...
    void bundle() {
        ...
        if (!(entry ==~ /[A-Za-z_$][\w$]*(\.[A-Za-z_$][\w$]*)+/)) {
            throw new GradleException('voidModule.entryPoint must be a fully qualified class name.')
        }
        if (minSdk.get() < 26) {
            throw new GradleException('VoidAndroid requires minSdk >= 26 for in-memory DEX loading.')
        }
        if (!runtimeDependencies.empty) {
            throw new GradleException('The first VoidAndroid module format does not bundle runtime dependencies. ' +
        ...
        new ZipFile(jar).withCloseable { classes ->
            ...
            if (classes.entries().any { it.name.startsWith('com/lelloman/voidandroid/api/') }) {
        ...
        new ZipFile(dexZip).withCloseable { result ->
            if (result.getEntry('classes2.dex') != null) {
                throw new GradleException('The initial VoidAndroid format supports a single DEX file only.')
            }
